...
//1. Parse email to generate the username.
	...
	$domain = "@" . $emailParts[1];	// Stick the @ back onto the domain since it was chopped off.
...
$v6  = "'" . $conn -> real_escape_string($emailGAPI) . "'";	//email from Google API
$v7  = "'" . $conn -> real_escape_string($username) . "'";	//Username parsed from Google API
$v8  = "'" . $conn -> real_escape_string($domain) . "'"; 	//User signed up with
$v9  = "'" . $conn -> real_escape_string($firstName) . "'"; //First Name from Google API
$v10 = "'" . $conn -> real_escape_string($lastName) . "'"; 	//Last Name from Google API

 $sqlsel="SELECT * FROM members WHERE email=$v6";	//Check to see if the risk has already been added to the existing alternative.
...
//4.b If user exists - user with this email doesnt exist yet, respond accoundingly..
		...
		if (!$conn -> query($sqlins)) {
			...
			$to      = $emailGAPI; // Send email to our user
