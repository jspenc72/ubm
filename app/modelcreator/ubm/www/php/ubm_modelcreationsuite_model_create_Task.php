...
if ($conn -> query($sqlins) === false) {
	...
	$sqlins2 = "INSERT INTO ubm_model_has_tasks (model_id, task_id) VALUES ( $v2, $last_inserted_id )";				//Create relationship between the model and the task
	if ($conn -> query($sqlins2) === false) {
		trigger_error('Wrong SQL: ' . $sqlins2 . ' Error: ' . $conn -> error, E_USER_ERROR);
	} else {
		$sqlins3 = "INSERT INTO ubm_model_step_has_tasks (step_id, task_id) VALUES ( $v8, $last_inserted_id )";		//Create relationship between the task and its parent in the hierarchy, which is the Step.
		if ($conn -> query($sqlins3) === false) {
			trigger_error('Wrong SQL: ' . $sqlins3 . ' Error: ' . $conn -> error, E_USER_ERROR);			
				} else {
			echo $_GET['callback'] . '(' . "{'message' : 'Requested Task $title was created successfully and added to model id: $activeModelId !'}" . ')';
		}
	}
}
