...
$sqlins = "INSERT INTO ubm_modelcreationsuite_heirarchy_object_antiSolipsism_UUID (procedure_id, created_by) VALUES ($v6,$v9)";
...
//1. Create specification of the checklistItem
    $sqlins = "INSERT INTO ubm_model_checklistItems (Instance_UUID, created_by) VALUES ($last_inserted_procedure_UUID, $v9)";
    if ($conn->query($sqlins) === false) {
        trigger_error('Wrong SQL: ' . $sqlins . ' Error: ' . $conn->error, E_USER_ERROR);
    } else {
    ...
    //2. Create instance of the checklistItem specification
        ...
        //3. Create Self Link of the checklistItem instance
            ...
            //3. Create Self Link of the Procedure instance
            $sqlins = "INSERT INTO ubm_modelcreationsuite_heirarchy_object_closureTable ( ancestor_id, descendant_id, path_length) VALUES ( $last_inserted_procedure_UUID,$last_inserted_procedure_UUID, '0')";
            if ($conn->query($sqlins) === false) {
                trigger_error('Wrong SQL: ' . $sqlins . ' Error: ' . $conn->error, E_USER_ERROR);
            } else {
            $affected_rows = $affected_rows + $conn -> affected_rows;
            ...
            if ($conn->query($sqlins3) === false) {
                ...
                echo $_GET['callback'] . '(' . "{'message' : 'Requested Procedure: $activeProcedureId was created successfully and linked to ChecklistItem: $last_inserted_checklistItem_UUID. This checklist item was then attached to Checklist: $activeChecklistUUID ! Affected Rows with Closure Table: $affected_rows'}" . ')';
            }
        }
    }
}
