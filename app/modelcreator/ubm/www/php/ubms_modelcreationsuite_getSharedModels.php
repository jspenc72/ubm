		...
		if($rs === false) {
		  trigger_error('Wrong SQL: ' . $sqlsel . ' Error: ' . $conn->error, E_USER_ERROR);
			...
			while ($items = $rs->fetch_assoc()) {
					...
					WHERE ubm_modelcreationsuite_heirarchy_object_antiSolipsism_UUID.UUID='$returnedModelUUID' AND ubm_model.Soft_DELETE!='TRUE'";		//Select all information for each model that was in the first result set...
