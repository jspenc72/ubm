...
//1. Select all records for checklist items stored in model_creation_suite, Count the number of items in the checklist.
			$sqlsel1="SELECT * FROM ubm_model_has_jobDescriptions WHERE model_id=$activeModelId AND jobdescription_id=$activeJobDescriptionId";		//Select all 
			...
			if($rs1 === false) {
			  trigger_error('Wrong SQL: ' . $sqlsel1 . ' Error: ' . $conn->error, E_USER_ERROR);
				...
				while ($items1 = $rs1->fetch_assoc()) {
					...
					$sqlsel2="SELECT * FROM ubm_model_jobdescription_has_policies WHERE jobdescription_id=$returnItemId";		//Select all 
					$rs2=$conn->query($sqlsel2);
					if($rs2 === false) {
					  trigger_error('Wrong SQL: ' . $sqlsel2 . ' Error: ' . $conn->error, E_USER_ERROR);
					} else {
						while ($items2 = $rs2->fetch_assoc()) {
							$returnItem2Id = stripslashes($items2['policy_id']);
							$sqlsel3="SELECT * FROM ubm_model_policies WHERE id=$returnItem2Id";		//Select all 
