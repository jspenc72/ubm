...
$v3 = "'" . $conn -> real_escape_string($category) . "'";
$v4 = "'" . $conn -> real_escape_string($description) . "'";
$v5 = "'" . $conn -> real_escape_string($activeModelId) . "'";
$v6 = "'" . $conn -> real_escape_string($username) . "'";

$sqlins = "INSERT INTO ubm_model_swot_analysis_elements (title, category, description, created_by) VALUES ( $v2, $v3, $v4, $v6 )"; 	//Creates a New Customer record.
if ($conn -> query($sqlins) === false) {
	trigger_error('Wrong SQL: ' . $sqlins . ' Error: ' . $conn -> error, E_USER_ERROR);
} else {
	$last_inserted_id = $conn -> insert_id;
	//$affected_rows = $conn -> affected_rows;
	$sqlins2 = "INSERT INTO ubm_model_has_swot_analysis_elements (model_id, swot_analysis_element_id, created_by) VALUES ( $v5, $last_inserted_id, $v6 )";
...
echo $_GET['callback'] . '(' . "{'message' : ' $category $title created successfully!'}" . ')';
