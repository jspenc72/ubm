...
function getMyModelsCustomers() { //Populates Customer  Listview on Model Settings Page
    ...
    $.getJSON('http://api.universalbusinessmodel.com/ubms_modelcreationsuite_model_getCurrentModel_Customers.php?callback=?', { //JSONP Request
        ...
        $.each(res, function(i, item) {
            ...
            $('#ubmsuite_modelSettings_myCustomers_ul').append("<li><a href='#'> <h2 style='white-space:normal;' title='" + item.name + "'>" + item.name + "</h2><p>" + item.name + "</p></a><a href='#ubmsuite_modelSettings_confirm_remove_Customer_popup' data-rel='popup' data-position-to='window' data-transition='pop' onclick='setActiveCustomerId(" + item.id + ")'>Remove Customer</a></li>");
