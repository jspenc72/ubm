...
$sqlins = "INSERT INTO ubm_model_alternative_cons (model_id, alternative_id, description, roi_ref, cost_low, cost_high, created_by) VALUES ( $v2, $v3, $v4, $v5, $v6, $v7, $v8 )"; //Creates a New Core Value record.
					...
					while ($items2 = $rs2->fetch_assoc()) {
						$sumLow = $items2['cost_low_sum'];
						 ...
						 trigger_error('Wrong SQL: ' . $sqlupd . ' Error: ' . $conn->error, E_USER_ERROR);
							 ...
							 $affected_rows = $conn->affected_rows;
							echo $_GET['callback'] . '(' . "{'message' : 'Requested Alternative Con $alternativeConDescription was created successfully, the new sum of low cost estimate is $sumLow and sum of high cost estimate is $sumHigh added to model id: $activeModelId affected rows: $affected_rows!'}" . ')';						 
						 }				
					}
				}		
			}
