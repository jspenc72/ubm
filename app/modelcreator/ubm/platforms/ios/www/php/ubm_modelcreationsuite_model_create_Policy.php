...
if ($conn -> query($sqlins) === false) {
	...
	$sqlins2 = "INSERT INTO ubm_model_has_policies (model_id, policy_id) VALUES ( $v2, $last_inserted_id )";
	if ($conn -> query($sqlins2) === false) {
		trigger_error('Wrong SQL: ' . $sqlins2 . ' Error: ' . $conn -> error, E_USER_ERROR);
	} else {
			$sqlins3 = "INSERT INTO ubm_model_jobdescription_has_policies (jobdescription_id, policy_id) VALUES ($v7, $last_inserted_id)";		//Create relationship between the task and its parent in the hierarchy, which is the Step.
