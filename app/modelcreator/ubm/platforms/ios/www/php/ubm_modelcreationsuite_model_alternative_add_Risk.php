...
//INSERT
$v3 = "'" . $conn -> real_escape_string($activeAlternative) . "'";
$v4 = "'" . $conn -> real_escape_string($activeAlternativeRisk) . "'";
$v5 = "'" . $conn -> real_escape_string($username) . "'";

 $sqlsel="SELECT * FROM ubm_model_alternative_has_risks WHERE alternative_id=$v3 AND risk_id=$v4";	//Check to see if the risk has already been added to the existing alternative.
 $rs=$conn->query($sqlsel);
 if($rs === false) {
 trigger_error('Wrong SQL: ' . $sqlsel . ' Error: ' . $conn->error, E_USER_ERROR);
 } else {
 	$rows_returned = $rs->num_rows;
	if($rows_returned>0){																			//If risk - alternative relationship already exists, respond accordingly.
			echo $_GET['callback'] . '(' . "{'message' : 'This risk has already been added to this alternative. Select another risk to continue.'}" . ')';
	}else{																							//If risk - alternative relationship does not already exist, create the relationship and respond accordingly.
		$sqlins = "INSERT INTO ubm_model_alternative_has_risks (alternative_id, risk_id, created_by) VALUES ( $v3, $v4, $v5 )";
		if ($conn -> query($sqlins) === false) {
			trigger_error('Wrong SQL: ' . $sqlins . ' Error: ' . $conn -> error, E_USER_ERROR);
		} else {
			$affected_rows = $conn -> affected_rows;
			echo $_GET['callback'] . '(' . "{'message' : 'The number of affected rows is $affected_rows. the Alternative modified was $activeAlternative.'}" . ')';
