 ...
 $itemId = $_GET['itemId']; 
 $setActiveReviewedByTime = $_GET['setActiveReviewedByTime']; 
...
//UPDATE
		...
		$sql="UPDATE `model_creation_suite_has_prepared_by_records` SET start_time='$setActiveReviewedByTime' WHERE model_id='$activeModelId' AND task_id='$itemId' ";
		if($conn->query($sql) === false) {
		  trigger_error('Wrong SQL: ' . $sql . ' Error: ' . $conn->error, E_USER_ERROR);
		} else {
		  $affected_rows = $conn->affected_rows;
		  echo $_GET['callback'] . '(' . "{'message' : 'The number of affected rows is $affected_rows, the modelId is: $activeModelId and the item id is is: $itemId the time set was $setActiveReviewedByTime!'}" . ')';
		}
/*Send Email Message 
