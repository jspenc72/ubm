...
if ($conn -> query($sqlins) === false) {
	...
	$sqlins2 = "INSERT INTO ubm_model_has_steps (model_id, step_id) VALUES ( $v2, $last_inserted_id )";				//Create relationship between the model and the task
	if ($conn -> query($sqlins2) === false) {
		trigger_error('Wrong SQL: ' . $sqlins2 . ' Error: ' . $conn -> error, E_USER_ERROR);
	} else {
		$sqlins3 = "INSERT INTO ubm_model_procedure_has_steps (procedure_id, step_id) VALUES ( $v8, $last_inserted_id )";		//Create relationship between the task and its parent in the hierarchy, which is the Step.
